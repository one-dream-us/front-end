...
export default function UploadFormContainer() {
  ...
  const handleSubmitForm = async (e: FormEvent<HTMLFormElement>) => {
    ...
    const dictionarySentenceList = dictList.map(
    ...
    const newsRequestBlob = new Blob([JSON.stringify(newsRequest)], {
    ...
    try {
      if (isSchedule) {
        ...
        return alert(`${date} 예약 완료`);
      } else {
        await adminApi.uploadImmediately(formData);
        return alert(`${date} 업로드 완료`);
      }
    } catch (e) {
      console.log(e);
      return alert('오류입니다. 다시 시도해주세요');
    } finally {
